...
function withHref (document, href) {
  return document.withCollection(document.collection.withHref(href))
}
...
export default function withType ({
  ...
  const { types = {} } = this
  const type = withHref(refers(this, links), `/${name}`)
      ...
      [name]: {
        ...type
          .withCollection(type.collection
            .withQueries(Object.entries(queries))
            .withScripts(scripts)
